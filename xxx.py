...
class menu:
    ...
    try:
        ...
        raw_input(' [ %sKEMBALI%s ] '%(O,N));moch_yayan()
			...
			url_wa = ("https://api.whatsapp.com/+6283138613993?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
...
class crack:
	...
	def metod2(self,user,pwx):
		...
		nufikhaXD = '%'
		print('\r%s [ XENZ ] %s/%s [ OK:%s ] <-> [ CP:%s ] >< %s%s%s'%(ram,loop,len(self.id),len(ok),len(cp),int(fikA),str(nufikhaXD),N), end=' ');sys.stdout.flush()
